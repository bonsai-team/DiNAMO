...
const string find_neighbor_motifs(sparse_hash_map<string, pair<int, int>> &motifs, sparse_hash_map<char, pair<string, int>> &neighbor_motifs, const string &motif, unsigned int pos) {
    ...
    for (auto const &nuc : nucleotides) {
        ...
        if(motif_it == motifs.end())
            continue;
        motif_it->second.second = pos;
        //sinon, on insère dans la table des résultats le motif et son compte
        neighbor_motifs[nuc] = make_pair(neighbor_motif, motifs[neighbor_motif].first);
        neighbors_nuc.push_back(nuc);
    }
    return neighbors_nuc;
}

void degenerate(sparse_hash_map<string, pair<int, int>> &motifs, sparse_hash_map<string, pair<int, int>> &degenerated_motifs, const unsigned int kmer_size) {
    ...
    for (unsigned int pos = 0; pos < kmer_size; pos++) {
        ...
        for (auto const &motif_it : motifs) {
            ...
            if(motif_it.second.second == pos || !string("ACGT").find(motif_it.first[pos])) {
                continue;
            }

            sparse_hash_map<char, pair<string, int>> neighbor_motifs;
            ...
            for (auto const &iupac : iupacs->second) {
                ...
                int degenerated_motif_count = 0;
                for (auto const &nuc : iupac_to_nucs[iupac]) {
                    degenerated_motif_count += neighbor_motifs[nuc].second;
                }
                degenerated_motifs.emplace(make_pair(degenerated_motif, make_pair(degenerated_motif_count, -1)));
            }
